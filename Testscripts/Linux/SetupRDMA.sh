...
function Install_OFED {
	...
	case $DISTRO in
		...
		;;
		centos_7 | redhat*)
			distro_version=$(echo "$DISTRO_VERSION" | cut -d '.' -f 1,2)
			mlx5_ofed_link="$mlx_ofed_partial_link$distro_version-x86_64.tgz"
		;;
		ubuntu*)
			distro_name=$ID
		...
		suse_15)
			distro_name=$ID
			distro_version=${VERSION_ID}sp0
			mlx5_ofed_link="$mlx_ofed_partial_link$distro_name$distro_version-x86_64.tgz"
		;;
		suse*)
			distro_name=$ID
			distro_version=${VERSION_ID%.*}sp${VERSION_ID##*.}
			mlx5_ofed_link="$mlx_ofed_partial_link$distro_name$distro_version-x86_64.tgz"
		;;
		*)
	...
	case $DISTRO in
		centos_7 | redhat*)
			...
			KERNEL=${KERNEL[-1]}
			./${file_nm%.*}/mlnxofedinstall --kernel $KERNEL --kernel-sources /usr/src/kernels/${KERNEL} --add-kernel-support --skip-repo --skip-unsupported-devices-check --without-fw-update
		;;
		centos_8)
		...
		ubuntu*)
			if [[ $DISTRO_VERSION == "20.04" ]]; then
				./${file_nm%.*}/mlnxofedinstall --add-kernel-support --skip-unsupported-devices-check --without-fw-update --force
			else
				./${file_nm%.*}/mlnxofedinstall --add-kernel-support --without-fw-update --force
			fi
		;;
		*)
			./${file_nm%.*}/mlnxofedinstall --add-kernel-support --skip-unsupported-devices-check --without-fw-update
		;;
	esac
	Verify_Result
...
function Main() {
	...
	# skip setup if using hpc images
	if [[ $usehpcimage == "yes" ]]; then
		LogMsg "Using HPC Image, skip set up RMDA."
		update_repos
		...
		if [[ $endure_sku == "yes" && $DISTRO == 'centos_7' && $mj -eq 7 && $mn -gt 6 ]]; then
		...
		case $DISTRO in
				...
				if [[ "$endure_sku" == "no" ]];then
					...
					case $DISTRO in
						redhat_7|centos_7)
							...
							yum install -y http://olcentgbl.trafficmanager.net/centos/${RELEASE}/updates/x86_64/kernel-devel-${KERNEL}.rpm
							yum install -y kernel-devel-${KERNEL}
							LogMsg "$?: Updated kernel-devel"
						;;
						redhat_8|centos_8|almalinux_8|rockylinux_8)
							...
							LogMsg "$?: Updated kernel-modules-extra"
						;;
						*)
							req_pkg="kernel-devel-$(uname -r)"
							install_package $req_pkg
						;;
					esac

					req_pkg="redhat-rpm-config rpm-build gcc gcc-gfortran libdb-devel gcc-c++ glibc-devel zlib-devel numactl numactl-devel binutils-devel iptables-devel libstdc++-devel libselinux-devel elfutils-devel libtool java libstdc++.i686 gtk2 atk cairo tcl tk createrepo byacc.x86_64 net-tools tcsh fuse-libs"
					install_package $req_pkg
			...
			suse*|sles*)
				...
				;;
			ubuntu*)
				LogMsg "Starting RDMA setup for Ubuntu$VERSION_ID"
				...
				LogMsg "Install DPDK for AN via system package ..."
				# the old fix integrated to dpdk-18.11 repo
				# this this not needed for ubuntu 16 and 18, even worse, it causes ubuntu16 to fail MPI test due to missing IB packages
				# add-apt-repository ppa:canonical-server/dpdk-azure-18.11 -y
				if [[ $DISTRO == "ubuntu_16.04" ]]; then
					LogMsg "$DISTRO not supported. Skipinstalling DPDK"
				else
					# add-apt-repository ppa:canonical-server/backup-dpdk-azure-18.11 -y
					if [[ $DISTRO == "ubuntu_18.04" ]]; then
						add-apt-repository ppa:canonical-server/server-backports -y
						if [ $? -ne 0 ]; then
							LogErr "Failed to add the required server-backports repo to apt source"
						else
							LogMsg "Successfully added the server-backports repo"
						fi
						update_repos
					fi
					apt-get install -y dpdk
					Verify_Result
				fi
				...
				if [[ "$endure_sku" == "no" ]];then
					LogMsg "Loading mlx5_ib modules ib_ipoib rdma_ucm"
					modprobe mlx5_ib ib_ipoib rdma_ucm
				...
				# excluding rdmacm-utils due to apt-get install failure when running upgrade test
				req_pkg="build-essential python-setuptools libibverbs-dev bison flex ibverbs-utils net-tools libdapl2 bc numactl tcl tk"
				install_package $req_pkg
