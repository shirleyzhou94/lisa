...
class DeviceFeatures:
    ...
    @classmethod
    def from_raw_feature(
        cls, device_name: str, device_feature_raw: str
    ) -> "DeviceFeatures":
        matched_features_info = DeviceFeatures._feature_info_pattern.search(
            device_feature_raw
        )
        if not matched_features_info:
            raise LisaException(f"Cannot get {device_name} features settings info")
...
class Ethtool(Tool):
    ...
    def get_device_enabled_features(
        ...
        device.device_features = DeviceFeatures.from_raw_feature(
...
class EthtoolFreebsd(Ethtool):
    ...
    def get_device_list(self, force_run: bool = False) -> Set[str]:
        devices = self.node.tools[Ifconfig].get_interface_list()
    ...
    def get_device_enabled_features(
        ...
        interface_info = self.node.tools[Ifconfig].run(interface).stdout
